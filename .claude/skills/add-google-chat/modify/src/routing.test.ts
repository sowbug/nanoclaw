...
describe('JID ownership patterns', () => {
  ...
  it('Google Chat JID: starts with gchat:', () => {
    const jid = 'gchat:abc123def';
    expect(jid.startsWith('gchat:')).toBe(true);
  });

  it('Google Chat space JID: starts with gchat: followed by space ID', () => {
    const jid = 'gchat:AAAAbcDeFgH';
    expect(jid.startsWith('gchat:')).toBe(true);
  });
});
